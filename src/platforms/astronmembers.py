...
def _safe_redirect_target(resp):
    # Some Astronmembers portals return Location headers with raw Latin-1 bytes
    # (e.g. an em-dash sent as \xe2 instead of percent-encoded UTF-8), which
    # crashes requests' default get_redirect_target on a UTF-8 decode. Fall
    # back to percent-encoding the offending bytes so the redirect is usable.
    if not resp.is_redirect:
        return None
    location_bytes = resp.headers["location"].encode("latin-1")
    try:
        return location_bytes.decode("utf-8")
    except UnicodeDecodeError:
        return quote(location_bytes, safe=b":/?#[]@!$&'()*+,;=~-._%")
...
""".strip()
    ...
    def authenticate(self, credentials: Dict[str, Any]) -> None:
        ...
        session.get_redirect_target = _safe_redirect_target
    ...
    def _parse_courses_from_html(self, html_content: str, base_url: str) -> List[Dict[str, str]]:
        ...
        for carousel in soup.find_all("div", class_="box-slider-cursos"):
            for link_tag in carousel.select("div.swiper-slide a[href]"):
                ...
                all_courses.append({"title": title, "url": full_url, "slug": slug, "id": slug, "seller_name": "Astronmembers"})
