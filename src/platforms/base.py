...
def sanitize_token(token: str) -> str:
    """
    Sanitizes a token by removing non-ASCII characters that can cause
    encoding issues in HTTP headers (Latin-1 codec errors).

    This handles common issues from copy-paste operations like:
    - Zero-width spaces
    - BOM characters
    - Smart quotes
    ...
    if not token:
        return token
    # Keep only ASCII printable characters (0x20-0x7E) which are safe for HTTP headers
    return ''.join(char for char in token if 0x20 <= ord(char) <= 0x7E)
