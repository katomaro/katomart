...
# Base URLs for Estrategia Militares API (same MCI backend as LDI)
API_BASE = "https://api.estrategia.com"
MILITARES_ORIGIN = "https://militares.estrategia.com"
LOGIN_URL = "https://perfil.estrategia.com/login"
PLAYWRIGHT_LOGIN_URL = LOGIN_URL
LOGIN_COMPLETE_URL = "https://militares.estrategia.com/objetivos/"

# API Endpoints (shared MCI system)
SEARCH_GOALS_URL = f"{API_BASE}/bff/goals/shelves"
GOAL_LDI_CONTENTS_URL = f"{API_BASE}/bff/goals/{{goal_id}}/contents/ldi"
...
class EstrategiaMilitaresTokenFetcher(PlaywrightTokenFetcher):
    """Automates Estrategia Militares login with a real browser to capture the bearer token."""
    ...
    async def fetch_token_async(
        ...
        *,
        ...
        """Custom fetch for Estrategia Militares - waits for manual login then captures token.
        ...
        async with async_playwright() as playwright:
            ...
            def on_new_page(new_page):
                ...
                if LOGIN_COMPLETE_URL in new_page.url or "militares.estrategia.com" in new_page.url:
                    login_complete = True
            ...
            try:
                ...
                logger.info(
                    "Aguardando login manual. Complete o login, abra a pagina de Militares "
                    "e clique OK quando estiver pronto..."
                )

                # Wait for login to complete (token captured or new tab with Militares)
                for _ in range(600):  # 10 minutes max
                    all_pages = context.pages
                    for p in all_pages:
                        if p.url.startswith(LOGIN_COMPLETE_URL) or "militares.estrategia.com/objetivos" in p.url:
                            login_complete = True
                            break
                    ...
                    if login_complete:
                        logger.info("Pagina de Militares detectada. Aguardando token...")
                ...
                if not captured_token:
                    raise ValueError(
                        ...
                        "e abrir a pagina de Militares (nova aba) antes de clicar OK."
                    )
...
class EstrategiaMilitaresPlatform(BasePlatform):
    """Implements the Estrategia Militares (LDI) platform."""
    ...
    def __init__(self, api_service: ApiService, settings_manager: SettingsManager) -> None:
        ...
        self._token_fetcher = EstrategiaMilitaresTokenFetcher()
    ...
    def auth_instructions(cls) -> str:
        return """
Como obter as credenciais do Estrategia Militares:

1. Acesse https://militares.estrategia.com e faca login normalmente.
...
7. No menu lateral, clique em Cookies > militares.estrategia.com.
...
""".strip()
    ...
    def requires_search(cls) -> bool:
        """Militares platform requires search to find content."""
        return True
    ...
    def authenticate(self, credentials: Dict[str, Any]) -> None:
        ...
        self._session.headers.update({
            ...
            "Origin": MILITARES_ORIGIN,
            "Referer": f"{MILITARES_ORIGIN}/",
        ...
        if token:
            ...
            logger.info("Sessao autenticada no Estrategia Militares via token.")
            return
        ...
        if secure_sid:
            ...
            self._configure_api_session()
            self._validate_session()
            logger.info("Sessao autenticada no Estrategia Militares via cookie __Secure-SID.")
            return
        ...
        if self._settings.has_full_permissions and use_browser_emulation:
            ...
            self._configure_api_session()
            self._validate_session()
            logger.info("Sessao autenticada no Estrategia Militares via emulacao de navegador.")
            return
    ...
    def _exchange_credentials_for_token(
        ...
        try:
            ...
            raise ConnectionError(
                "Falha ao obter o token do Estrategia Militares via emulacao de navegador. "
    ...
    def _validate_session(self) -> None:
        ...
        try:
            ...
            logger.debug("Sessao Militares validada com sucesso.")
        except Exception as exc:
            logger.warning("Falha ao validar sessao Militares: %s", exc)
    ...
    def fetch_courses(self) -> List[Dict[str, Any]]:
        """
        For Militares, returns an empty list since search is required.
        Users must use search_courses to find content.
        """
        logger.info("Militares requer busca para localizar conteudos. Use a funcao de pesquisa.")
        return []
    ...
    def search_courses(self, query: str) -> List[Dict[str, Any]]:
        ...
        if not query or not query.strip():
            logger.warning("Militares requer uma consulta de pesquisa.")
            return []
        ...
        try:
            ...
            logger.error("Falha ao buscar conteudos Militares: %s", exc)
            return []
        ...
        logger.info("Encontrados %d cursos Militares para '%s'.", len(courses), query)
        return courses
    ...
    def fetch_course_content(self, courses: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for course in courses:
            ...
            try:
                ...
                logger.error("Falha ao obter detalhes do curso Militares %s: %s", course_slug, exc)
                continue
            ...
            course_entry = {
                "id": str(course_id),
                "name": course_data.get("name", course.get("name", "Curso Militares")),
                "slug": course_slug,
                "title": course_data.get("name", course.get("name", "Curso Militares")),
                "modules": modules,
    ...
    def mark_lesson_watched(self, lesson: Dict[str, Any], watched: bool) -> None:
        ...
        logger.debug("Marcar item Militares %s como visualizado: %s", lesson.get("id"), watched)


PlatformFactory.register_platform("Estrategia Militares LDI", EstrategiaMilitaresPlatform)
